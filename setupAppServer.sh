...
installAllure(){
  cd ~
  wget https://repo.maven.apache.org/maven2/io/qameta/allure/allure-commandline/2.14.0/allure-commandline-2.14.0.tgz
  sudo tar -xzvf allure-commandline-2.14.0.tgz
  echo "export PATH=$PATH:/home/ubuntu/allure-2.14.0/bin" | sudo tee -a ~/.bashrc
}
...
installS3FS() {
  ...
  mkdir ~/spb-test-reports
  sudo tee -a /etc/systemd/system/s3fs.service <<EOT
[Unit]
Description=Mount S3 file system
After=network.target

[Service]
Type=forking

ExecStart=s3fs spb-test-reports /home/ubuntu/spb-test-reports -o iam_role="App_QA_Role",uid=1000,gid=1000 -o nonempty
